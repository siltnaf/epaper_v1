...
StoryItem stories[ITEMS_PER_PAGE] = {};
uint8_t storyCount = 0;
int32_t storyTotal = 0;
...
void markDirtyRow(int8_t index) {
    if (index < 0 || index >= static_cast<int8_t>(storyCount) || dirtyRowA == index ||
...
void updateReaderPagination() {
    ...
    Serial.printf("[STORY] Reader pagination bytes=%u pages=%u lines_per_page=%d\n",
                  selectedContent.length(), readerPageTotal, READER_LINES_PER_PAGE);
}

bool ensureStoryDirectory(int32_t storyId, char *directory, size_t directorySize) {
    if (!SdCard::isMounted() || storyId <= 0 || !directory || directorySize == 0) return false;
    if (!SD_MMC.exists(STORY_SD_FOLDER) && !SD_MMC.mkdir(STORY_SD_FOLDER)) return false;
    snprintf(directory, directorySize, "%s/%ld", STORY_SD_FOLDER, static_cast<long>(storyId));
    return SD_MMC.exists(directory) || SD_MMC.mkdir(directory);
}

bool isStorySavedOnSd(int32_t storyId) {
    if (!SdCard::isMounted() || storyId <= 0) return false;
    char audioPath[96] = {};
    return cachedOpusPath(storyId, audioPath, sizeof(audioPath));
}

void saveStoryMetadata(int32_t storyId, const char *title) {
    char directory[40] = {};
    if (!ensureStoryDirectory(storyId, directory, sizeof(directory))) return;
    char metaPath[64] = {};
    snprintf(metaPath, sizeof(metaPath), "%s/meta.txt", directory);
    SD_MMC.remove(metaPath);
    File meta = SD_MMC.open(metaPath, FILE_WRITE);
    if (!meta) return;
    meta.printf("%ld\n%s\n\n\n", static_cast<long>(storyId), title ? title : "");
    meta.close();
}

bool saveStoryToSd(int32_t storyId, const char *title, const char *author,
                  const char *category, const String &content) {
    char directory[40] = {};
    if (!ensureStoryDirectory(storyId, directory, sizeof(directory))) {
        Serial.printf("[STORY SD] Could not create cache folder for id=%ld\n", static_cast<long>(storyId));
        return false;
    }
    ...
    if (written != content.length()) {
        SD_MMC.remove(temporaryPath);
        Serial.printf("[STORY SD] Short write id=%ld expected=%u written=%u\n",
                      static_cast<long>(storyId), content.length(), written);
        return false;
    }
    ...
    metaFile.flush();
    metaFile.close();
    Serial.printf("[STORY SD] Saved id=%ld bytes=%u folder=%s\n",
                  static_cast<long>(storyId), content.length(), directory);
    return true;
}

bool loadStoryFromSd(const StoryItem &story) {
    if (!SdCard::isMounted() || story.id <= 0) return false;
    char directory[40] = {};
    snprintf(directory, sizeof(directory), "%s/%ld", STORY_SD_FOLDER, static_cast<long>(story.id));
    char metaPath[64] = {};
    char contentPath[64] = {};
    snprintf(metaPath, sizeof(metaPath), "%s/meta.txt", directory);
    snprintf(contentPath, sizeof(contentPath), "%s/content.txt", directory);
    ...
    metaFile.close();
    copyUtf8(selectedTitle, sizeof(selectedTitle), title.c_str(), story.title);
    ...
    view = View::Player;
    Serial.printf("[STORY SD] Loaded id=%ld bytes=%u from %s\n",
                  static_cast<long>(story.id), selectedContent.length(), directory);
    return true;
}
...
String endpointBase() {
    ...
    const int stories = base.indexOf("/api/voice-stories");
    if (stories >= 0) return base.substring(0, stories) + "/api/voice-stories";
...
bool httpGet(const String &url, String &payload, uint32_t timeoutMs = 20000,
    ...
    if (WiFi.status() != WL_CONNECTED) {
        ...
        Serial.println("[STORY] WiFi is not connected");
        return false;
    }
...
bool ensureStoryOpus(char *path, size_t pathSize) {
    if (cachedOpusPath(selectedStoryId, path, pathSize)) {
        Serial.printf("[STORY TTS] Using voice cache storyId=%ld voice=%s path=%s\n",
    ...
    char directory[40] = {};
    if (!ensureStoryDirectory(selectedStoryId, directory, sizeof(directory))) return false;

    String metadataUrl = endpointBase() + "/" + String(selectedStoryId) +
                         "/tts?voice=" + String(selectedVoice);
    Serial.printf("[STORY TTS] Metadata request storyId=%ld voice=%s url=%s\n",
                  static_cast<long>(selectedStoryId), selectedVoice, metadataUrl.c_str());
    ...
    for (uint8_t attempt = 1; attempt <= 3 && !metadataLoaded; ++attempt) {
        ...
        if (!metadataLoaded && attempt < 3) {
            Serial.printf("[STORY TTS] Metadata retry attempt=%u/3\n",
    ...
    if (error) {
        Serial.printf("[STORY TTS] Metadata JSON failed: %s\n", error.c_str());
        return false;
    }
    ...
    if (!downloadFile(downloadUrl, path) || !cachedOpusPath(selectedStoryId, path, pathSize)) {
        return false;
    }
    return true;
}
...
bool startStoryAudio() {
    ...
    storyPlaying = OpusPlayer::play(path);
    std::strcpy(audioStatus, storyPlaying
...
bool loadLibrary(bool showLoading = true, bool forceRemote = false,
    ...
    if (!remoteLoaded) {
        ...
        File root = SD_MMC.open(STORY_SD_FOLDER);
        ...
        while (entry) {
            if (entry.isDirectory()) {
                ...
                if (isStorySavedOnSd(id)) {
                    if (validIndex >= firstItem && storyCount < ITEMS_PER_PAGE) {
                        StoryItem &story = stories[storyCount];
                        story.id = id;
                        story.saved = true;
                        char metaPath[64] = {};
                        snprintf(metaPath, sizeof(metaPath), "%s/%ld/meta.txt",
                                 STORY_SD_FOLDER, static_cast<long>(id));
                        ...
                        if (meta) {
                            ...
                            meta.close();
                            char fallback[24] = {};
                            snprintf(fallback, sizeof(fallback), "STORY %ld", static_cast<long>(id));
                            copyUtf8(story.title, sizeof(story.title), title.c_str(), fallback);
                            ++storyCount;
                        }
                    }
                    ++validIndex;
        ...
        root.close();
        if (storyCount > 0) {
            snprintf(statusText, sizeof(statusText), "%u SAVED STORIES", storyCount);
            Serial.printf("[STORY SD] Offline library page=%ld items=%u total=%ld\n",
                          static_cast<long>(libraryPage), storyCount,
                          static_cast<long>(storyTotal));
            return true;
        }
        std::strcpy(statusText, "NO ONLINE OR SAVED STORIES");
        return false;
    }
    Serial.printf("[STORY] Library payload preview: %.180s\n", payload.c_str());
    JsonDocument document;
    const DeserializationError error = deserializeJson(document, payload);
    if (error) {
        if (fetchedRemote) return loadLibrary(showLoading, false, true);
        snprintf(statusText, sizeof(statusText), "JSON %s", error.c_str());
        Serial.printf("[STORY API] JSON parse failed: %s\n", error.c_str());
        return false;
    }
    storyTotal = document["total"] | 0;
    JsonArray items = document["items"].as<JsonArray>();
    if (items.isNull()) {
        if (fetchedRemote) return loadLibrary(showLoading, false, true);
        std::strcpy(statusText, "NO ITEMS IN JSON");
        Serial.println("[STORY API] Parsed JSON has no items array");
        return false;
    }
    if (fetchedRemote) PlaylistCache::save(
        STORY_SD_FOLDER, endpoint, cacheSlot, payload);
    for (JsonObject item : items) {
        if (storyCount >= ITEMS_PER_PAGE) break;
        StoryItem &story = stories[storyCount];
        story.id = item["id"] | 0;
        char fallback[24] = {};
        snprintf(fallback, sizeof(fallback), "STORY %ld", static_cast<long>(story.id));
        copyUtf8(story.title, sizeof(story.title), item["title"] | "", fallback);
        story.saved = isStorySavedOnSd(story.id);
        Serial.printf("[STORY API] parsed item index=%u id=%ld title=%s\n",
                      storyCount, static_cast<long>(story.id), story.title);
        if (story.id > 0) ++storyCount;
    }
    if (storyCount == 0) {
        std::strcpy(statusText, "NO STORIES FOUND");
        return false;
    }
    snprintf(statusText, sizeof(statusText), "%u STORIES", storyCount);
    return true;
}
...
bool loadStory(const StoryItem &story) {
    ...
    String payload;
    bool loaded = false;
    const String url = endpointBase() + "/" + String(story.id);
    for (uint8_t attempt = 1; attempt <= 3 && !loaded; ++attempt) {
        Serial.printf("[STORY] Detail fetch id=%ld attempt=%u/3\n",
                      static_cast<long>(story.id), attempt);
        loaded = httpGet(url, payload);
        if (!loaded && attempt < 3) delay(800);
    }
    if (!loaded) {
        std::strcpy(statusText, "STORY LOAD FAILED");
        return false;
    }
    JsonDocument document;
    const DeserializationError error = deserializeJson(document, payload);
    if (error) {
        Serial.printf("[STORY API] Detail JSON parse failed: %s\n", error.c_str());
        std::strcpy(statusText, "STORY JSON FAILED");
        return false;
    }
    selectedStoryId = document["id"] | story.id;
    copyUtf8(selectedTitle, sizeof(selectedTitle), document["title"] | "", story.title);
    ...
    if (selectedContent.isEmpty()) {
        std::strcpy(statusText, "STORY HAS NO CONTENT");
        return false;
    }
    readerPage = 0;
    updateReaderPagination();
...
int libraryPageCount() {
    return storyTotal > 0 ? (storyTotal + ITEMS_PER_PAGE - 1) / ITEMS_PER_PAGE : 1;
}
...
void renderLibrary(uint8_t *frame) {
    if (UiLocalization::isChinese()) drawUtf8Title(frame, 78, 34, 84, 18, "讲故事");
    else drawCentered(frame, 36, "VOICE STORIES", 2);
...
void renderPlayer(uint8_t *frame) {
    ...
    if (UiLocalization::isChinese()) {
        drawCentered(frame, 112, "讲故事", 2);
        drawCentered(frame, 158, "语音");
    } else {
        drawCentered(frame, 112, "VOICE STORY", 2);
        drawCentered(frame, 158, "TTS VOICE");
    }
    drawCentered(frame, 180, selectedVoice, 2);
    rect(frame, 70, 228, 100, 100);
    drawPlayPause(frame, 120, 278, storyPlaying);
    drawCentered(frame, 350, audioStatus);
}
...
bool startLibraryLoad() {
    ...
    libraryLoadCompleted = false;
    if (xTaskCreate(libraryLoadTask, "voice-library", 4096, nullptr, 1, nullptr) != pdPASS) {
        libraryLoadRunning = false;
        libraryAwaitingContent = false;
    ...
    if (view == View::Library) {
        ...
        for (uint8_t index = 0; index < storyCount; ++index) {
            ...
            if (pointInRect(x, y, 12, top, 216, ROW_HEIGHT)) {
                if (activeStoryIndex == static_cast<int8_t>(index)) {
                    markDirtyRow(activeStoryIndex);
                    stopAudio();
                    return true;
                }
                const int8_t previousIndex = activeStoryIndex;
                stopAudio();
                markDirtyRow(previousIndex);
                selectedStoryId = stories[index].id;
                copyUtf8(selectedTitle, sizeof(selectedTitle), stories[index].title, "VOICE STORY");
                selectedAuthor[0] = '\0';
                selectedCategory[0] = '\0';
                selectedContent = "";
                std::strcpy(audioStatus, UiLocalization::isChinese() ? "正在准备语音" : "PREPARING AUDIO");
                saveStoryMetadata(selectedStoryId, selectedTitle);
                activeStoryIndex = static_cast<int8_t>(index);
                markDirtyRow(activeStoryIndex);
                marqueeReady = false;
                marqueeOffset = 0;
                // Start after this tap-triggered display refresh completes, so
                // the SD-backed Opus decoder never races the SD-backed font renderer.
...
bool isAudioActive() {
    return pendingAudioStart || storyPlaying || OpusPlayer::isPlaying();
}
...
bool advanceMarquee(int16_t &rowTop) {
    if (!storyPlaying || activeStoryIndex < 0 || !marqueeReady || millis() < nextMarqueeMs) {
        return false;
    }
    marqueeOffset = (marqueeOffset + 12) % MARQUEE_WIDTH;
    nextMarqueeMs = millis() + 900;
    rowTop = LIST_TOP + activeStoryIndex * (ROW_HEIGHT + ROW_GAP);
    return true;
}
...
bool handleSwipe(int16_t deltaX, int16_t deltaY) {
    ...
    if (view == View::Library) {
        if (next && libraryPage < libraryPageCount()) {
            ++libraryPage;
            loadLibrary();
            Serial.printf("[STORY SWIPE] Library next page=%ld axis=%s\n",
        ...
        if (!next && libraryPage > 1) {
            --libraryPage;
            loadLibrary();
            Serial.printf("[STORY SWIPE] Library previous page=%ld axis=%s\n",
                          static_cast<long>(libraryPage), horizontal ? "horizontal" : "vertical");
            return true;
        }
        return false;
