...
struct TPoint
{
	...
	bool aboutEqual(const TPoint point)
	{
		...
		return abs(static_cast<int32_t>(x) - point.x) <= maxDeviation &&
			   abs(static_cast<int32_t>(y) - point.y) <= maxDeviation;
	}
};
...
private:
	...
	uint8_t _chipId = 0xFF;
	uint8_t _vendorId = 0xFF;
	uint8_t _firmwareVersion = 0xFF;
