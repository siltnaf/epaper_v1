...
void XingtaiEpd::displayPartialFill(const uint8_t *oldFrame, uint8_t fill,
                                   uint16_t x, uint16_t y,
                                   uint16_t width, uint16_t height) {
    if (oldFrame == nullptr || width == 0 || height == 0 || x >= WIDTH || y >= HEIGHT) {
        return;
    }
    ...
    const size_t rowBytes = WIDTH / 8;
    const size_t windowBytes = (logicalXEnd - logicalXStart + 1) / 8;
    uint8_t fillRow[WIDTH / 8];
    memset(fillRow, fill, sizeof(fillRow));
